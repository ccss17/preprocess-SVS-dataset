...
class LyricNormalizer:
    ...
    def normalize_lyrics(self, gt_lyrics: str, raw_lyrics: str, pitch_sequence: List[int], 
                         duration_sequence: List[int], normalize_spaces: bool = True) -> Dict:
        """
        입력 텍스트를 정규화하여 반환합니다.
        ...
        pitch_sequence, duration_sequence, space_indices = self.remove_silence_from_pitch_and_duration(pitch_sequence, duration_sequence)
        
        # 2. DTW를 통한 정렬
        _, _, _, syllable_mapping = self.dtw(gt_lyrics, raw_lyrics, space=False)
        ...
        for gt_idx, raw_indices in syllable_mapping.items():
            if len(raw_indices) == 1:
                # 1:1 매핑 처리
                lyrics, pitches, durations, infos = self.normalize_one_to_one_mapping(gt_syllables, gt_idx, raw_indices, pitch_sequence, duration_sequence, current_idx)
            else:
                # 1:N 매핑 처리
                lyrics, pitches, durations, infos = self.normalize_one_to_many_mapping(gt_syllables, gt_idx, raw_syllables, raw_indices, pitch_sequence, duration_sequence, current_idx)
        ...
        if normalize_spaces:
            insertion_points = self.find_space_insertion_points(raw_lyrics, normalization_infos, space_indices)
            normalized_lyrics, normalized_pitches, normalized_durations, normalization_infos = self.insert_spaces(
                normalized_lyrics, normalized_pitches, normalized_durations, normalization_infos, insertion_points
            )
    ...
    def find_space_insertion_points(self, raw_text: str, normalization_infos: List[Dict], space_indices: List[Tuple[int, int]]) -> List[Dict]:
    ...
    def insert_spaces(self, normalized_texts: List[str], normalized_pitches: List[int], 
                     normalized_durations: List[int], normalization_infos: List[Dict], insertion_points: List[Dict]) -> Tuple[List[str], List[int], List[int], List[Dict]]:
        """정규화된 텍스트에 공백을 삽입합니다."""
        ...
        return normalized_texts, normalized_pitches, normalized_durations, normalization_infos
    ...
    def normalize_one_to_one_mapping(self, gt_syllables: List[str], gt_idx: int, raw_indices: List[int], 
                                     pitch_sequence: List[int], duration_sequence: List[int], current_idx: int) -> Tuple[List[str], List[int], List[int], List[Dict]]:
        """1:1 매핑을 처리합니다."""
        ...
        return [gt_syllable], [pitch_sequence[raw_indices[0]]], [duration_sequence[raw_indices[0]]], [normalized_info]

    def normalize_one_to_many_mapping(self, gt_syllables: List[str], gt_idx: int, raw_syllables: List[str], 
                                   raw_indices: List[int], pitch_sequence: List[int], duration_sequence: List[int], current_idx: int) -> Tuple[List[str], List[int], List[int], List[Dict]]:
        """1:N 매핑을 처리합니다."""
        if not self.is_pitch_change(pitch_sequence, raw_indices):
            ...
            normalized_info = {
                ...
                'normalized_syllable': gt_syllable,
                'normalized_duration': sum(duration_sequence[raw_indices[0]:raw_indices[-1]+1]),
                'gt_idx': gt_idx,
                'raw_indices': raw_indices,
                'normalized_idx': current_idx
            }
            ...
            return [gt_syllable], [pitch_sequence[raw_indices[0]]], [normalized_duration], [normalized_info]   
        else:
            split_indices = self.split_indices_by_pitch_change(raw_indices, pitch_sequence)
            return self.normalize_one_to_many_mapping_by_pitch_change(gt_syllables, gt_idx, raw_syllables, split_indices, pitch_sequence, duration_sequence, current_idx)
    ...
    def normalize_one_to_many_mapping_by_pitch_change(self, gt_syllables: List[str], gt_idx: int, raw_syllables: List[str], 
                                                    split_indices: List[Tuple[int, int]], pitch_sequence: List[int], duration_sequence: List[int], current_idx: int) -> Tuple[List[str], List[int], List[int], List[Dict]]:
        # 각 청크 처리
        normalized_texts = []
        normalized_pitches = []
        normalized_durations = []
        ...
        for i, (start, end) in enumerate(split_indices):
            ...
            normalized_durations.append(sum(duration_sequence[start:end+1]))
            normalization_infos.append({
                ...
                'normalized_duration': sum(duration_sequence[start:end+1]),
                'gt_idx': gt_idx,
    ...
    def remove_silence_from_pitch_and_duration(self, pitch_sequence, duration_sequence):
        new_pitch_sequence = []
        new_duration_sequence = []
        space_indices = []
        
        if len(pitch_sequence) != len(duration_sequence):
            raise ValueError("pitch_sequence and duration_sequence must have the same length")
        else:
